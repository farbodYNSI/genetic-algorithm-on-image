...
def initialPopulation(nIndividuals=8):
    init_population = np.empty(shape=(nIndividuals,imageWidth*imageHeight),dtype=np.uint8)
    for indv_num in range(nIndividuals):
        # Randomly generating initial population chromosomes genes values.
        init_population[indv_num, :] = np.random.randint(0, 256, imageWidth*imageHeight)
...
def crossover(parents, nIndividuals=8):
    ...
    new_population = np.empty(shape=(nIndividuals, imageWidth*imageHeight), dtype=np.uint8)
    ...
    # Selecting some parents randomly from the permutations for offsprings
    ...
    for comb in range(len(selected_permutations)):
        ...
        # Crossover by desired percent between 2 parents
        cross_size = np.int32(new_population.shape[1]*crossover_percent/100)
...
def showAndSave(iter , resultPop,originalImg):
    result = resultPop[0].reshape((imageWidth,imageHeight))
    evaluate = np.ones_like(result)
    evaluate[result != originalImg]=0
    fig, axs = plt.subplots(2)
    axs[0].imshow(result,cmap='gray')
    axs[1].imshow(image,cmap='gray')
    axs[0].text(evaluate.shape[0],evaluate.shape[1],'pixelsRatio = '+str(int(np.sum(evaluate)/(evaluate.shape[0]*evaluate.shape[1])*100)))
    figName = str(iter)
    if saveFigures:
        plt.savefig(figName)
    plt.show(False)
...
imageVector=img2vector(image)
population=initialPopulation(sol_per_pop)

for iteration in range(maxIter):
    qualities = calPopFitness(imageVector, population)
    print('Quality : ', qualities, ', Iteration : ', iteration)
    parents = getBestParents(population, qualities, num_parents_mating)
    population = crossover(parents, sol_per_pop)
    population = mutation(population, num_parents_mating, mutation_percent)
    
    if qualities[0]==0:
        showAndSave(iteration,population,image)
        break

    if iteration == 100 or iteration == 1000 or iteration ==10000 or iteration ==50000 or iteration ==100000:
        showAndSave(iteration,population,image)
else:        
    showAndSave(maxIter,population,image)
